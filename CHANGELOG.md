...
- [~] 声骸名容错匹配: **已实现, 工作区未提交**(阶段十一); 已知失效(错字丢字→`[]` 或子串抢跑误配)保留为图标识别的兜底
- [ ] **套装图标识别(下一棒)**: 名字多义(120/181 属多套) + OCR 错字无法在名字层消歧 → 改读游戏内套装图标。图标素材已就位(`assets/echo_icons/{套装名}.png`, 34 个), 方案、待补坐标与待确认问题见 `CLAUDE.md` → "接力状态 / 下一棒"
