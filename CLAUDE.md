...
- 评估/渐进判定(`judge_echo` 公共, **五档**): **Lv5/Lv10 = 结构判定**——存在"首条核心词条"(`_core_first`, 缺省=全部有效词条; 通用 weight>0)即过, 不限分。Lv15+ 两条线: **达标线 A = 10 × 出现有效词条权重之和**; **基准线 B = 该套装有效键最低 (tier-1) 条之和×10**(配置未定制的套装其 B 即通用线 11/18/26.5; 定制的套装用低权重"占位键"压 B 是刻意取向, 不被通用标准覆盖)。
  **达标 = score ≥ B 且 A ≥ B 且 score ≥ A**(三条同时); **A < B 说明出现有效词条太少、求和项不足以支撑达标 → 该只最高只能到"保留"**。数学: k(=出现有效词条数) ≥ tier-1 时 A ≥ B 恒成立; k < tier-1 时 A 可能 < B(实测 34% 样本 k < tier-1, 其中 25 只真的更低)。
  满级: ≥A(且A≥B) → **达标**; ≥B → **保留**; **建议重铸 = ≥2 条词条达/超平均档(档位值/均值 ≥1.0) 或 四有效打底(eff≥4)**(胚子语义: 锁住 ≥2 条好底子, 或只剩 1 条要改; **「好词条」按档位水平算, 不看该套装是否认** —— 重铸后适配性可改, 故「双暴满档+3废」算 2 条好底子 → 建议重铸; 依据 README 旧注「单条高档位不保留(洗 4 条不值)」); 否则 **不合格**。未满级: ≥B → **待强化**; 否则 **不合格**。**强化流程不豁免**(不达标仍丢弃), 保留/建议重铸仅评估报告标注
