...
- 评估/渐进判定(`judge_echo` 公共): **Lv5/Lv10 = 结构判定——存在"首条核心词条"(`_core_first`, 缺省=全部有效词条; 通用 weight>0)即过, 不限分**; Lv15/20/25 = 总分 ≥ 锚线 `_tier_threshold` = **max(本只"出现的有效词条"(套装权重>0, 且真的出现)的 10×权重平均值 × (tier-1), 下限)**; **下限 = max(套装有效键最低 tier-1 条之和, 通用代表集 11/18/26.5)** —— 兜住"有效词条少且权重低"的极端(否则新公式会算出 4.0 这类比旧规则还松的线; 只取"旧规则(套装)"不够, 它对键多权重低的套装本身就塌陷)。**旧规则单独使用已废弃**: 它让"键多但权重低"的套装锚线极低(实测 轻云出月 9 键/4 个专伤 0.15 → Lv25=6.0; 隐世回光 9 键/生命防御 0.1~0.15 → 5.0), 变成"词条种类越多越易达标"; **keep(满级不达标且有效条数≥2且score≥18) → 评估报告标"建议保留重铸", 强化不豁免**
