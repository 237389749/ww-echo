...
- 评估/渐进判定(`judge_echo` 公共, **五档**): **Lv5/Lv10 = 结构判定**——存在"首条核心词条"(`_core_first`, 缺省=全部有效词条; 通用 weight>0)即过, 不限分。Lv15+ 用两条线: **达标线 A = max(10 × 出现有效词条权重之和, 旧规则线 B)**; **旧规则线 B = 该套装有效键最低 (tier-1) 条之和×10**(配置未定制的套装其 B 即通用线 11/18/26.5)。k(=出现有效词条数) ≥ tier-1 时 A ≥ B 恒成立; k 小时 A 求和项少、可能 < B → max 兜底(实测 34% 样本 k < tier-1, 其中 25 只 A 真的更低)。**不引入通用线作额外下限**: 套装配置被定制即为刻意取向(如 隐世回光/轻云出月 用低权重"占位键"压低基准线, 因为真正核心词条少), 不该被通用标准覆盖。满级: score ≥ A → **达标**; score ≥ B → **保留**; 有效≥2 且 ≥18 → **建议重铸**; 否则 **不合格**。未满级: score ≥ B → **待强化**; 否则 **不合格**。**强化流程不豁免**(不达标仍丢弃), 保留/建议重铸仅评估报告标注
