...
- **数据来源/工具备注**: 声骸清单由 wuther.in 页面(用户浏览器保存)解析——条目左上角 `IconElementAttri*.webp` ↔ 筛选按钮"图标+套装名"; 解析脚本 `tools/parse_wutherin_echoes.py <页面.html>`(只覆盖解析到数据的套装, **保留手补值**, 自动补 `_icon`/新增套, 生成 `echoes_check.md`); **kurobbs/wuther.in 均不可脚本爬取**(前者要令牌, 后者 robots.txt Disallow AI 爬虫); 当前 34 套(31 原 + 3 新), 仅 `剪心辑梦之影` 缺清单(待用户补)
