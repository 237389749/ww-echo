...
名字天生多义(**181 个声骸名中 120 个属 ≥2 套装**) + OCR 错字, 名字层无法完全消歧。已实证失效: `梦魔·青羽鹭`(清洗后含子串「青羽鹭」且该名独立存在于索引 → 子串分支抢跑 → 误配 `[啸谷长风, 浮星祛暗]`, 应 `[息界同调之律]`)、`侏侏驼`(驼→鸵, 清洗后仅「侏侏」2 字)、`咔嘧`/`阿磁磁`(真名 咔嚓嚓/阿嗞嗞, 清洗后仅 1 字)。详见 CHANGELOG 阶段十一。**这类只能靠套装图标识别兜底**(见"接力状态")。

## 评估遍历 v2 现状与约束

- 前置: 游戏已在**背包声骸列表界面**(无需预选)
- 流程: 网格 OCR `+xx` 角标 → 聚行 → 6 列基准(跨屏记忆 `col_centers`)补全缺列防漏 → 逐格点选 → 右侧详情 OCR → 档位过滤词条 → 评分/截图/记录
- **评估按声骸名自动映射套装**: 套装模板含 `_echoes: {4c:[...], 3c:[...], 1c:[...]}`(声骸清单, 数据来自 wuther.in 页面图标→套装对照, 见下); 评估时 `get_set_by_echo(声骸名)` → 用该套装的权重/键/首核, **未录入回退"通用"**(如无图标的 冰盈舞者/共鸣回响类)。一个声骸可属多套装(120/181) → 名字容错给候选, `get_set_by_echo` 取首选(见"声骸名容错匹配"段)
- 套装语境(权重/键/首核/声骸清单)供评估自动映射 + 强化模式使用; run_tab 评估仍隐藏套装下拉(靠映射), 未映射时用通用
- **多套装声骸归属**: `get_sets_by_echo(名)` 名字容错 → **候选套装列表**(120/181 声骸属多套); `get_set_by_echo(名, prefer=config套装)` — prefer 在候选内则尊重它, 否则取首个; **下一步: 读游戏内套装图标消歧**(素材已备 `assets/echo_icons/{套装名}.png` 34 个, 方案/待补坐标见"接力状态"); 套装模板含 `_icon`(如 Cloud/Ice)供核对
...
- 报告(`_build_eval_html`): 名称独立列 + 得分区间/判定多选/名称包含筛选(原生 JS) + 词条按档位 ratio 单色相连续渐变着色(`hue=210`, 亮度 92%→46%, **越深越高档**; 白底细描边)
- **已知缺陷(原版 ok-ww 同款)**: 强化 `run()` 无"切下一只"推进, 依赖丢弃后游戏自动前移; 评估遍历在网格布局/字体变化时坐标常量需复核

## 接力状态 / 下一棒(2026-09-12)

**工作区未提交**: `src/echo_set_templates.py` 的声骸名容错匹配(见 CHANGELOG 阶段十一)。用户要求**与套装图标识别一起做完再提交** → 接手请勿单独提交该文件。

**下一棒 = 套装图标识别(名字→套装消歧的硬信号)**
- 为什么: 名字天生多义(120/181 属多套) + OCR 错字, 名字层无法区分(实证见 CHANGELOG 阶段十一); 游戏内套装图标是唯一硬信号
- 流程: 详情面板裁套装图标区 → **二值化(Otsu, 去颜色/抗锯齿)** → 尺寸归一化 → 与 `assets/echo_icons/{套装名}.png`(**34 个**)做 `matchTemplate`/pHash → 唯一高置信套装即定; 否则回退名字候选(`get_sets_by_echo`)
- **缺输入(当前阻塞)**: 图标区**归一化坐标**——本机无图像理解模型, 需用户/视觉模型提供:
  - A. 详情面板内**套装图标** bbox `x1,y1,x2,y2`(归一化 0-1), 及**等级文字 `+25`** bbox(作"图标紧邻等级"的参照)
  - B. **背包页声骸卡片**上套装名/图标的归一化坐标(兜底用)
- **待用户确认**: 背包卡片上是否**直接印着套装名/图标**? 若是, 可跳过名字 OCR 直接读卡片, 免去图标模板匹配
- 兜底(错字三层策略): ①图标筛选样本(主) ②名称字典去错字(已实现) ③背包页直接读套装名
