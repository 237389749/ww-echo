...
<h3>达标线（锚线）</h3>
<p><b>达标线 A = max（ 10 × 本只"出现的有效词条"的权重之和， 基准线 B ）</b>（n = 词条数，L = n−1）。<br>
"有效" = 该套装启用、权重 &gt; 0 的词条；<b>基准线 B</b>（旧规则）= 该套装有效键最低 L 条之和 × 10
（配置未定制的套装即通用线 <b>11 / 18 / 26.5</b>）。<br>
含义：A 要求"这只有效词条每条都达到自己的平均档位"（平均档 = 有价值）；出现有效词条数 k ≥ L 时
<b>A ≥ B 恒成立</b>，k &lt; L 时由 B 兜底（避免求和项变少导致线过低）。<br>
Lv5 / Lv10 为结构判定：只要存在"首条核心词条"即通过，不限分。</p>
<h3>判定（五档）</h3>
<p>两条线：<b>达标线 A</b>（平均档水平）、<b>基准线 B</b>（旧规则线）。满级按下表逐级命中：</p>
<ul>
<li><b>达标</b>：总分 ≥ A（必然也不低于 B）</li>
<li><b>保留</b>：总分 ≥ B —— 过了基准门槛，值得留着</li>
<li><b>建议重铸</b>：有效词条 ≥2 且 总分 ≥18（洗练成本线，锁 2 追 3）</li>
<li><b>不合格</b>：其余</li>
</ul>
