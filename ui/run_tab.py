...
<h3>达标线（锚线）</h3>
<p>Lv15 / Lv20 / Lv25 的达标线 = <b>max（ 本只"出现的有效词条"的"平均档加权分"平均值 × (n−1)， 下限 ）</b>
（n = 词条数；"有效" = 该套装启用、权重 &gt; 0 的词条）。<br>
<b>下限</b> = max（该套装有效键最低 n−1 条之和，通用代表集最低 n−1 条之和 = <b>11 / 18 / 26.5</b>）——
保证"再严也不会比通用标准松"，也避免"有效词条少又都是低权重"时算出 4.0 这类漏网的线。<br>
即：出现的词条质量越高，本只的门槛也越高；不再单独用"套装最低 n−1 条"定线（那会让键多但权重低的套装被误放宽）。<br>
