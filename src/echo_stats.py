...
# 通用默认权重(尺制定稿: 条分 = 档位/均值 ×10×权重, 上限自然 12.5; 套装模板权重优先, 此表仅"通用"兜底):
# 暴击/爆伤最高; 百分比三系(攻/生/防)统一 0.85(考虑生命/防御模板); 共效 0.7;
# 专伤(普攻/重击/共技/共解)0.6; 固定三系统一 0.5(白值700-1000下小攻击≈1%攻, 且对全乘区通用, 优于不完善专伤)。
# 有效词条集合均由用户按套装预设(weight>0 即计入)。
DEFAULT_WEIGHTS: dict[str, float] = {
    "暴击": 1.0, "暴击伤害": 0.9,
    "攻击百分比": 0.85, "生命百分比": 0.85, "防御百分比": 0.85,
    "共鸣效率": 0.7,
    "普攻伤害加成": 0.6, "重击伤害加成": 0.6,
    "共鸣技能伤害加成": 0.6, "共鸣解放伤害加成": 0.6,
    "攻击": 0.5, "生命": 0.5, "防御": 0.5,
}
