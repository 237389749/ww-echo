...
- **按声骸名自动映射套装**: 每套装可配置其声骸清单(4C/3C/1C); 评估时自动用该声骸所属套装的权重/首核判分, 未录入的回退通用权重
...
- [~] **声骸名容错匹配**: 已实现(工作区未提交)——多套装声骸名(120/181 属多套)按白名单清洗 + 子串 + LCS 给候选; 已知错字失效见 CHANGELOG 阶段十一
- [ ] **套装图标识别(下一棒)**: 名字多义 + OCR 错字无法在名字层消歧 → 读游戏内套装图标裁决。图标素材已就位(`assets/echo_icons/`, 34 个), 方案与待补坐标见 `CLAUDE.md` → "接力状态 / 下一棒"
